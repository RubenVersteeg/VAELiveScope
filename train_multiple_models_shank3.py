...
removal_of_female_is_done = 'true'
removal_of_female = (removal_of_female_is_done.lower() == 'true')
...
if __name__ == "__main__":
    ...
    (
        ...
        main_freq,
        recording_path,
    ) = seperate_spec_and_features.separate_spec_and_features(usvs)

    labels, emitter_library = make_labels.make_labels(emitter, label_type='strain')
    padded_spec, original_indices, indices_after_padding, labels_after_padding = create_padded_spectograms.pad_specs(
        spec, 160, labels, removal_of_female=removal_of_female
    )
    ...
    del usvs, spec, emitter, distance_to_nearest_non_emitter, duration, start_time_in_recording, end_time_in_recording, intensities, intensity, location, main_freq, recording_path
    ...
    for j in range(number_of_models_per_latent_size):
        for i in latent_sizes:
            ...
            sub_folder = base_folder / f'shank3_{i}_v{j}'
            ...
            ) = load_usv_data_strain.load_data(
                normalized_padded_spec, labels_after_padding, indices_after_padding,
                batch_size=batch_size, oversample=method_is_oversample, nr_workers=9
            )
            ...
            save_model.save_parameters(
                spectograms_file, labels_file, batch_size, model_choice, sub_folder,
                latent_space_size, nr_epochs, amount_of_patience, slope_leaky,
                oversampling_method, learning_rate, precision_model,
                max_value_is_per_spec, removal_of_female_is_done
            )
